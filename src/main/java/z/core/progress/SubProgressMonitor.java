...
public class SubProgressMonitor extends ProgressMonitorWrapper {
    ...
    /**
     * Constructs a new sub-progress monitor for the given monitor. The sub
    ...
    public void beginTask(String name, int totalWork) {
        ...
        // this submonitor.
        scale = totalWork <= 0 ? 0.0 : (double) parentTicks / (double) totalWork;
    ...
    public void internalWorked(double work) {
        ...
        if (sentToParent >= (double) parentTicks) {
            usedUp = true;
        }
    }
    ...
    public void subTask(String name) {
        ...
        if ((style & PREPEND_MAIN_LABEL_TO_SUBTASK) != 0 && mainTaskLabel != null && mainTaskLabel.length() > 0) {
            super.subTask(mainTaskLabel + ' ' + name);
        } else {
            super.subTask(name);
        }
    }
